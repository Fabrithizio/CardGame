...
// Descrição: Feedback central minimalista. Usa apenas texto sobre a faixa do meio,
// sem o bloco pesado que quebrava a arena visual.
...
namespace CardGame.UI
{
    public class BattleCombatCenterUI : MonoBehaviour
    {
        ...
        [Header("Layout")]
        [SerializeField] private Vector2 anchorPosition = new Vector2(0f, -6f);
        [SerializeField] private Vector2 panelSize = new Vector2(900f, 124f);

        [Header("Timing")]
        [SerializeField] private float visibleDuration = 1.85f;
        [SerializeField] private float fadeDuration = 0.35f;

        [Header("Filtro")]
        [SerializeField] private bool showMythicMessages = false;
        [SerializeField] private bool showStateMessages = false;

        [Header("Texto")]
        [SerializeField] private int titleFontSize = 26;
        [SerializeField] private int subtitleFontSize = 17;
        [SerializeField] private int impactFontSize = 32;

        [Header("Cores")]
        [SerializeField] private Color titleColor = new Color(1f, 0.92f, 0.72f, 1f);
        [SerializeField] private Color subtitleColor = new Color(0.92f, 0.95f, 1f, 0.92f);
        [SerializeField] private Color damageColor = new Color(1f, 0.28f, 0.28f, 1f);
        [SerializeField] private Color healColor = new Color(0.32f, 1f, 0.48f, 1f);
        [SerializeField] private Color mythicColor = new Color(0.92f, 0.48f, 1f, 1f);
        ...
        private void HandleLogMessage(string condition, string stackTrace, LogType type)
        {
            ...
            if (ShouldIgnoreLog(condition)) return;

            if (TryParseCombatMessage(condition, out CombatMessage message))
            {
        ...
        private bool ShouldIgnoreLog(string log)
        {
            ...
            {
                if (clean.Contains("[Estado]") ||
                    clean.Contains("Fase atual") ||
                    clean.Contains("Turno iniciado") ||
                    clean.Contains("Turno encerrado") ||
                    clean.Contains("Debug ativo") ||
                    clean.Contains("Batalha iniciada") ||
                    clean.Contains("Míticos disponíveis") ||
                    clean.Contains("começou o turno") ||
                    clean.Contains("Seu turno") ||
                    clean.Contains("SEU TURNO") ||
                    clean.Contains("TURNO DO INIMIGO"))
                {
                    return true;
                }
            }

            if (!showMythicMessages && (clean.Contains("MÍTICO") || clean.Contains("Mítico") || clean.Contains("mítico")))
            {
                return true;
            }
        ...
        private bool TryParseCombatMessage(string log, out CombatMessage message)
        {
            ...
            if (showMythicMessages && clean.Contains("MÍTICO ATIVADO"))
            {
        ...
        private void CreateUI()
        {
            ...
            vertical.padding = new RectOffset(12, 12, 4, 4);
            vertical.spacing = -4f;
            ...
            titleText = CreateText(panelObject.transform, "Title", titleFontSize, FontStyle.Bold, 32f, titleColor);
            impactText = CreateText(panelObject.transform, "Impact", impactFontSize, FontStyle.Bold, 40f, damageColor);
            subtitleText = CreateText(panelObject.transform, "Subtitle", subtitleFontSize, FontStyle.Italic, 38f, subtitleColor);
        }
        ...
        private Text CreateText(Transform parent, string objectName, int fontSize, FontStyle style, float height, Color color)
        {
            ...
            rect.sizeDelta = new Vector2(panelSize.x - 24f, height);

            Text text = textObject.AddComponent<Text>();
            text.font = defaultFont;
            text.fontSize = Mathf.Clamp(fontSize, 10, 38);
            ...
            text.resizeTextMaxSize = Mathf.Clamp(fontSize, 10, 38);
            return text;
        }
